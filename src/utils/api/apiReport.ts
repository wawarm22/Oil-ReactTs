...
import { MaterialProductDistributionResponse, OilUseInProductsResponse, ProductFormulaResponse, RawMaterialPaymentsResponse } from "../../types/reportTypes";
...
export const getOilUseInProducts = async (
    ...
    auth: AuthSchema
): Promise<OilUseInProductsResponse | undefined> => {
    try {
...
export const getMaterialProductDistribution = async (
    params: {
        factory_slug: string,
        company_id: number,
        month: number,
    ...
    auth: AuthSchema
): Promise<MaterialProductDistributionResponse | undefined> => {
    try {
        const response = await axios.get(`${BASE_URL_AWS}/report/material-product-distribution`,
            {
                params,
                headers: {
                    Authorization: `Bearer ${auth.accessToken}`,
